...
int main(void) {
   ...
   *              uxPriority,      Priority (0 = idle, configMAX_PRIORITIES-1 =
   * highest) pxCreatedTask )  Optional handle output (NULL if not needed)
   */
  xTaskCreate(prvSenderTask, "Sender", configMINIMAL_STACK_SIZE, NULL,
              SENDER_TASK_PRIORITY, NULL);

  xTaskCreate(prvReceiverTask, "Receiver", configMINIMAL_STACK_SIZE, NULL,
              RECEIVER_TASK_PRIORITY, NULL);
