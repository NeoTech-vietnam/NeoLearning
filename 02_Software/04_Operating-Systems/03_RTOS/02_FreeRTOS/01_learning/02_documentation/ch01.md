...
FreeRTOS is a collection of C libraries comprising a real-time kernel and a set of modular libraries that implement complementary functionality.
...
Allocating priorities in that way would ensure hard real-time threads always execute ahead of soft real-time threads, but priority assignment decisions are not always that simplistic.
