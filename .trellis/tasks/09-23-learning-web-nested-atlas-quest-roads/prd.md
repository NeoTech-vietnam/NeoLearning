...
2. Layout and hit regions are stable for a given content tree and consistent with the current cartographic art. No AI imagery, random-looking geography, or duplicate topic text.
...
- [ ] Deeper navigation, zoom-out, refresh and browser back/forward restore the correct view.
...
Primary risk: nested maps may look artificial. Do not mass-produce procedural geography before the pilot is visually approved. The existing list is a fallback; no content node may disappear because layout metadata is missing. Preserve unrelated uncommitted changes.
