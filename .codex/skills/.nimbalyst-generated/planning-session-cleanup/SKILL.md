...
Use this when the user explicitly invokes `/planning-session-cleanup` or requests the "session-cleanup" workflow.

Important: treat the text after `/planning-session-cleanup` as the command arguments. Use those arguments anywhere the original Claude command expects its argument placeholder.
